...
**Relative-first is not a style choice.** A top-level `alphaearth_change`, once imported, stays
in `sys.modules` for the life of the QGIS process. Reinstalling the plugin purges only the
plugin's OWN package (`alphaearth_change_qgis.*`), so the top-level engine from the PREVIOUS
