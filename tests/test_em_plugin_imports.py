...
inside the plugin package. A top-level `from embed_cd import ...` survives in
...
PLUGIN = os.path.join(os.path.dirname(HERE), "plugin", "embed_cd_qgis")
TOP_LEVEL = re.compile(r"^\s*(from\s+embed_cd(\.|\s+import)|import\s+embed_cd\b)",
                       re.MULTILINE)
...
def test_engine_py_tries_the_vendored_copy_first():
    ...
    rel = text.index("from .embed_cd import")
    top = text.index("from embed_cd import")
...
def test_release_vendors_the_engine_next_to_the_plugin():
    ...
    assert '"change": ("embed_cd_qgis", "embed_cd")' in script
